...
#+
PUBLIC FUNCTION StakeholderZoom_uidialogdata_record1_clear()
...
PUBLIC FUNCTION StakeholderZoom_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL StakeholderZoom_uidialogdata_record1_clear()
    CALL StakeholderZoom_uidata.StakeholderZoom_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION StakeholderZoom_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL StakeholderZoom_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL StakeholderZoom_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION StakeholderZoom_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL StakeholderZoom_uidata.StakeholderZoom_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].clientstakeholders_stakeholderId) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION StakeholderZoom_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION StakeholderZoom_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.clientstakeholders_stakeholderId == m_record1_arrRecList[m_record1_keyIndex].clientstakeholders_stakeholderId THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.clientstakeholders_stakeholderId == m_record1_arrRecList[i].clientstakeholders_stakeholderId THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
