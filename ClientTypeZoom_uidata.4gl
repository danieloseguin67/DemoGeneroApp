...
PUBLIC FUNCTION ClientTypeZoom_uidata_record1_getDataArray(p_arr_br_fields, p_arr_db_fields, p_where, p_detailList)
    ...
    LET l_sqlQuery = 'SELECT ', l_distinct
        ...
        , p_where
        , ''
        , ' ORDER BY clienttypes.clientypename ASC '
    {<POINT Name="fct.record1_getDataArray.init">} {</POINT>}
    IF ClientTypeZoom_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL ClientTypeZoom_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
    ...
    WHENEVER ERROR CONTINUE
        DECLARE getDataArray_cursqlsid_ClientTypeZoom_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getDataArray_cursqlsid_ClientTypeZoom_record1 INTO
                p_arr_br_fields[i].clienttypes_clienttypeid,
                p_arr_br_fields[i].clienttypes_clientypename,
                p_arr_db_fields[i].*

            IF ClientTypeZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
                CALL ClientTypeZoom_events.m_DataEvent_record1_OnComputedFields(p_arr_br_fields[i])
            END IF
        ...
        FREE getDataArray_cursqlsid_ClientTypeZoom_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_fields.clear()
        CALL p_arr_db_fields.clear()
...
PUBLIC FUNCTION ClientTypeZoom_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    ...
    WHENEVER ERROR CONTINUE
        ...
        IF ClientTypeZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL ClientTypeZoom_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
