...
#+ @return p_arr_br_uk
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_getKeys(p_arr_br_uk, p_where, p_detailList)
    ...
    IF PreferencesControlTable_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL PreferencesControlTable_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
            RETURNING l_sqlQuery
    END IF

    WHENEVER ERROR CONTINUE
        DECLARE getKeys_cursqlsid_PreferencesControlTable_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getKeys_cursqlsid_PreferencesControlTable_record1 INTO l_rec_data.*
            LET p_arr_br_uk[p_arr_br_uk.getLength() + 1].* = l_rec_data.*
        END FOREACH
        FREE getKeys_cursqlsid_PreferencesControlTable_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_uk.clear()
    END IF
...
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    ...
    WHENEVER ERROR CONTINUE
        ...
        IF PreferencesControlTable_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL PreferencesControlTable_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
...
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_insertRow(p_rec_br)
    ...
    IF PreferencesControlTable_events.m_DataEvent_record1_BeforeInsertRow IS NOT NULL THEN
        CALL PreferencesControlTable_events.m_DataEvent_record1_BeforeInsertRow(dataInsert.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF PreferencesControlTable_events.m_DataEvent_record1_AfterInsertRow IS NOT NULL THEN
                CALL PreferencesControlTable_events.m_DataEvent_record1_AfterInsertRow(errNo, errMsg, ret.*)
...
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_updateRow(p_rec_br, p_dataT0)
    DEFINE p_rec_br record1_br_type
    ...
    IF PreferencesControlTable_events.m_DataEvent_record1_BeforeUpdateRow IS NOT NULL THEN
        CALL PreferencesControlTable_events.m_DataEvent_record1_BeforeUpdateRow(p_dataT0.*, l_dataT1.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF PreferencesControlTable_events.m_DataEvent_record1_AfterUpdateRow IS NOT NULL THEN
                CALL PreferencesControlTable_events.m_DataEvent_record1_AfterUpdateRow(errNo, errMsg, l_rec_br_uk.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_deleteRowWithConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE preferences.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    IF PreferencesControlTable_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess IS NOT NULL THEN
        CALL PreferencesControlTable_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess(p_dataT0.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF PreferencesControlTable_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess IS NOT NULL THEN
                CALL PreferencesControlTable_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess(errNo, errMsg, p_dataT0.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
#+ @returnType INTEGER, STRING
#+ @return     ERROR_SUCCESS|ERROR_FAILURE|ERROR_CONCURRENT_ACCESS_NOTFOUND|ERROR_CONCURRENT_ACCESS_FAILURE, error message
PUBLIC FUNCTION PreferencesControlTable_uidata_record1_checkRowConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE preferences.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
