...
IMPORT FGL RFSManagerform_uidialog
...
PUBLIC FUNCTION RFSManagerform_ui_uiOpenForm(p_whereRelation, p_uiSettings)
    ...
    CALL RFSManagerform_ui_uiOpenFormByKey(p_whereRelation, p_uiSettings.*, l_record1_br_uk.*) RETURNING errNo, actionNo, l_record1_br_uk.*
...
PUBLIC FUNCTION RFSManagerform_ui_uiOpenFormByKey(p_whereRelation, p_uiSettings, p_br_uk)
    DEFINE p_whereRelation STRING
    DEFINE p_uiSettings UISettings_Type
    ...
    OPEN WINDOW w_RFSManagerform WITH FORM 'RFSManagerform' ATTRIBUTES(STYLE=NVL(l_windowStyle, "main_dbapp"))
    --Initialize UI settings with default form UI settings
    CALL RFSManagerform_ui_initializeDefaultUISettings() RETURNING l_defaultUISettings.*
    ...
    IF RFSManagerform_events.m_DlgEvent_OnOpenForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent_OnOpenForm(w.getForm())
    END IF
    ...
    IF m_uiSettings.openMode != C_MODE_SEARCH AND m_uiSettings.openMode != C_MODE_EMPTY THEN
        ...
        LET m_record1_keyIndex = RFSManagerform_uidialogdata_record1_seek(p_br_uk.*)
    END IF
    --Call the UI automaton
    CALL RFSManagerform_ui_uiAutomaton(m_uiSettings.*) RETURNING errNo, actionNo
    INITIALIZE l_record1_br_uk.* TO NULL
    IF m_record1_keyIndex > 0 THEN
        LET l_record1_br_uk.rfs_rfsId = m_record1_arrRecList[m_record1_keyIndex].rfs_rfsId
    END IF
    CLOSE WINDOW w_RFSManagerform
...
PRIVATE FUNCTION RFSManagerform_ui_uiAutomaton(p_uiSettings)
    DEFINE p_uiSettings UISettings_Type
    ...
    WHILE runningAutomaton
        CASE l_state
            ...
            WHEN C_MODE_DISPLAY
                ...
                CALL RFSManagerform_ui_uiDisplay(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_MODIFY
                ...
                CALL RFSManagerform_ui_uiInput(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_SEARCH
                ...
                CALL RFSManagerform_ui_uiConstruct(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_EMPTY
                ...
                CALL RFSManagerform_ui_uiEmpty(prevState, prevAction, l_state) RETURNING errNo, actionNo
...
FUNCTION RFSManagerform_ui_uiDisplay(p_prevState, p_prevAction, p_state)
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG RFSManagerform_uidialog.RFSManagerform_uidialog_record1_uiDisplay
        ...
        ON ACTION new
            ...
            IF RFSManagerform_events.m_DlgEvent_OnActionNew IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionNew(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
        ...
        ON ACTION query
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF RFSManagerform_events.m_DlgEvent_OnActionQuery IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionQuery(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION accept
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF RFSManagerform_events.m_DlgEvent_OnActionAccept IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionAccept(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION cancel
            ...
            IF RFSManagerform_events.m_DlgEvent_OnActionCancel IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionCancel(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION close
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF RFSManagerform_events.m_DlgEvent_OnActionClose IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionClose(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION exit
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF RFSManagerform_events.m_DlgEvent_OnActionExit IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionExit(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
        ...
        BEFORE DIALOG
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.CONTINUE_DIALOG
            IF RFSManagerform_events.m_DlgEvent_Before_BrowseDialog IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_Before_BrowseDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction, m_NextField)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction, m_NextField
            END IF
        ...
        AFTER DIALOG
            ...
            IF RFSManagerform_events.m_DlgEvent_After_BrowseDialog IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_After_BrowseDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
...
#+ @return           Error number, 0-success, other-error
#+ @return           The next performed action
FUNCTION RFSManagerform_ui_uiInput(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    ...
    DEFINE l_br_uk_zm_offerstastus RECORD
        rfs_offerstatusid LIKE rfs.offerstatusid
    END RECORD
    DEFINE l_br_uk_zm_stakeholder RECORD
        rfs_stakeholderId LIKE rfs.stakeholderId
    END RECORD
    DEFINE l_br_uk_zm_client RECORD
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG RFSManagerform_uidialog.RFSManagerform_uidialog_record1_uiInput

        ON ACTION zm_offerstastus
            {<POINT Name="fct.uiInput.dlg.onAction.zm_offerstastus">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_offerstastus() RETURNING errNo, actionNo, l_br_uk_zm_offerstastus.*
            IF errNo == ERROR_SUCCESS THEN
                ...
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    IF m_record1_arrRecList.getLength() > 0 THEN
                        IF (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_offerstatusid IS NULL) OR (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_offerstatusid != l_br_uk_zm_offerstastus.rfs_offerstatusid) THEN
                            CALL DIALOG.setFieldTouched("record1.offerstatusid", TRUE)
                            LET m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_offerstatusid = l_br_uk_zm_offerstastus.rfs_offerstatusid
            ...
            END IF
        ON ACTION zm_stakeholder
            {<POINT Name="fct.uiInput.dlg.onAction.zm_stakeholder">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_stakeholder() RETURNING errNo, actionNo, l_br_uk_zm_stakeholder.*
            IF errNo == ERROR_SUCCESS THEN
                IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                    LET m_actionNo = C_ACTION_EXIT_APP
                    LET m_dataProcessing = TRUE
                ...
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    IF m_record1_arrRecList.getLength() > 0 THEN
                        IF (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_stakeholderId IS NULL) OR (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_stakeholderId != l_br_uk_zm_stakeholder.rfs_stakeholderId) THEN
                            CALL DIALOG.setFieldTouched("record1.stakeholderId", TRUE)
                            LET m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_stakeholderId = l_br_uk_zm_stakeholder.rfs_stakeholderId
                            --The 'NEXT FIELD CURRENT' is done to trigger existing 'ON CHANGE'.
                            --Especially in the following case:
                            --  - The value of the field is 'A'
                            --  - Select a value 'B' using the ZOOM functionality
            ...
            END IF
        ON ACTION zm_client
            {<POINT Name="fct.uiInput.dlg.onAction.zm_client">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_client() RETURNING errNo, actionNo, l_br_uk_zm_client.*
            IF errNo == ERROR_SUCCESS THEN
                IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                    LET m_actionNo = C_ACTION_EXIT_APP
                    LET m_dataProcessing = TRUE
                ...
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    IF m_record1_arrRecList.getLength() > 0 THEN
                        IF (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_clientid IS NULL) OR (m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_clientid != l_br_uk_zm_client.rfs_clientid) THEN
                            CALL DIALOG.setFieldTouched("record1.clientid", TRUE)
                            LET m_record1_arrRecList[DIALOG.getCurrentRow("record1")].rfs_clientid = l_br_uk_zm_client.rfs_clientid
                            --The 'NEXT FIELD CURRENT' is done to trigger existing 'ON CHANGE'.
                            --Especially in the following case:
                            --  - The value of the field is 'A'
                            --  - Select a value 'B' using the ZOOM functionality
        ...
        ON ACTION zoom
            ...
            IF INFIELD(record1.offerstatusid) THEN
                CALL RFSManagerform_ui_action_zm_offerstastus() RETURNING errNo, actionNo, l_br_uk_zm_offerstastus.*
                IF errNo == ERROR_SUCCESS THEN
                    IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                        LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
            END IF
            IF INFIELD(record1.stakeholderId) THEN
                CALL RFSManagerform_ui_action_zm_stakeholder() RETURNING errNo, actionNo, l_br_uk_zm_stakeholder.*
                IF errNo == ERROR_SUCCESS THEN
                    IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                        LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
            END IF
            IF INFIELD(record1.clientid) THEN
                CALL RFSManagerform_ui_action_zm_client() RETURNING errNo, actionNo, l_br_uk_zm_client.*
                IF errNo == ERROR_SUCCESS THEN
                    IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                        LET m_actionNo = C_ACTION_EXIT_APP
        ...
        ON ACTION cancel
            ...
            CALL RFSManagerform_ui_validateCRUDOperationWrapper(DIALOG, m_subdialog, m_state, C_ACTION_CANCEL)
        ...
        AFTER DIALOG
            INITIALIZE RFSManagerform_events.m_DlgCtrlInstruction TO NULL
            IF RFSManagerform_events.m_DlgEvent_After_EditDialog IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_After_EditDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION RFSManagerform_ui_uiConstruct(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG RFSManagerform_uidialog.RFSManagerform_uidialog_record1_uiConstruct

        ON ACTION zm_offerstastus
            {<POINT Name="fct.uiConstruct.dlg.onAction.zm_offerstastus">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_offerstastus() RETURNING errNo, actionNo, l_br_uk_zm_offerstastus.*
            IF errNo == ERROR_SUCCESS THEN
                IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                    LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    DISPLAY l_br_uk_zm_offerstastus.rfs_offerstatusid TO rfs.offerstatusid
                END IF
            ELSE
                MESSAGE "Error on the zoom"
            END IF
        ON ACTION zm_stakeholder
            {<POINT Name="fct.uiConstruct.dlg.onAction.zm_stakeholder">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_stakeholder() RETURNING errNo, actionNo, l_br_uk_zm_stakeholder.*
            IF errNo == ERROR_SUCCESS THEN
                IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                    LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    DISPLAY l_br_uk_zm_stakeholder.rfs_stakeholderId TO rfs.stakeholderId
                END IF
            ELSE
                MESSAGE "Error on the zoom"
            END IF
        ON ACTION zm_client
            {<POINT Name="fct.uiConstruct.dlg.onAction.zm_client">} {</POINT>}
            CALL RFSManagerform_ui_action_zm_client() RETURNING errNo, actionNo, l_br_uk_zm_client.*
            IF errNo == ERROR_SUCCESS THEN
                IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                    LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
                IF actionNo == C_ACTION_ACCEPT THEN --Handle the return value of the zoom
                    DISPLAY l_br_uk_zm_client.rfs_clientid TO rfs.clientid
                END IF
            ELSE
                MESSAGE "Error on the zoom"
            END IF
        ON ACTION zoom
            ...
            IF INFIELD(rfs.offerstatusid) THEN
                CALL RFSManagerform_ui_action_zm_offerstastus() RETURNING errNo, actionNo, l_br_uk_zm_offerstastus.*
                IF errNo == ERROR_SUCCESS THEN
                    IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                        LET m_actionNo = C_ACTION_EXIT_APP
                ...
                END IF
            END IF
            IF INFIELD(rfs.stakeholderId) THEN
                CALL RFSManagerform_ui_action_zm_stakeholder() RETURNING errNo, actionNo, l_br_uk_zm_stakeholder.*
                IF errNo == ERROR_SUCCESS THEN
                    IF actionNo == C_ACTION_EXIT_APP THEN --Exit Application was raised on the zoom, then the application should be closed without handle the return value of the zoom
                        LET m_actionNo = C_ACTION_EXIT_APP
        ...
        BEFORE DIALOG
            ...
            INITIALIZE RFSManagerform_events.m_DlgCtrlInstruction TO NULL
            IF RFSManagerform_events.m_DlgEvent_Before_SearchDialog IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_Before_SearchDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
            CASE RFSManagerform_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
        ...
        AFTER DIALOG
            ...
            INITIALIZE RFSManagerform_events.m_DlgCtrlInstruction TO NULL
            IF RFSManagerform_events.m_DlgEvent_After_SearchDialog IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_After_SearchDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction, m_where, m_detailList)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction, m_where, m_detailList
            END IF
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION RFSManagerform_ui_uiEmpty(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    DEFINE errNo INTEGER
    ...
    MENU
        BEFORE MENU
            ...
            END IF
        INITIALIZE RFSManagerform_events.m_DlgCtrlInstruction TO NULL
        IF RFSManagerform_events.m_DlgEvent_Before_EmptyDialog IS NOT NULL THEN
            CALL RFSManagerform_events.m_DlgEvent_Before_EmptyDialog(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                RETURNING RFSManagerform_events.m_DlgCtrlInstruction
        END IF
        ...
        ON ACTION new
            ...
            LET RFSManagerform_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_MENU
            IF RFSManagerform_events.m_DlgEvent_OnActionNew IS NOT NULL THEN
                CALL RFSManagerform_events.m_DlgEvent_OnActionNew(DIALOG, m_state, RFSManagerform_events.m_DlgCtrlInstruction)
                    RETURNING RFSManagerform_events.m_DlgCtrlInstruction
            END IF
...
PUBLIC FUNCTION RFSManagerform_ui_uiInitializeDefaultActions(p_actionList)
    ...
    LET p_actionList[14] = "zm_offerstastus"
    LET p_actionList[15] = "zm_stakeholder"
    LET p_actionList[16] = "zm_client"
...
PUBLIC FUNCTION RFSManagerform_ui_initializeDefaultUISettings()
    DEFINE l_uiSettings UISettings_Type
...
--------------------------------------------------------------------------------
--zoom open functions
{<BLOCK Name="fct.action_zm_offerstastus">}
#+ Launch the module
#+
#+
#+ @return None
FUNCTION RFSManagerform_ui_action_zm_offerstastus()
    DEFINE l_uiSettings UISettings_Type
    ...
    DEFINE errNo INTEGER
    DEFINE actionNo SMALLINT
    {<POINT Name="fct.action_zm_offerstastus.define">} {</POINT>}
    ...
    LET l_br_uk.rfs_offerstatusid = IIF(m_record1_keyIndex > 0, m_record1_arrRecList[m_record1_keyIndex].rfs_offerstatusid, NULL)
    INITIALIZE l_whereRelation TO NULL

    {<POINT Name="fct.action_zm_offerstastus.init">} {</POINT>}
    IF RFSManagerform_events.m_DlgEvent__action_zm_offerstastus_BeforeOpeningTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_offerstastus_BeforeOpeningTheForm(m_state)
    END IF

    CALL OfferStatusZoom_ui_uiOpenFormByKey(l_whereRelation, l_uiSettings.*, l_br_uk.*) RETURNING errNo, actionNo, l_br_uk.*

    IF RFSManagerform_events.m_DlgEvent__action_zm_offerstastus_AfterClosingTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_offerstastus_AfterClosingTheForm(m_state)
    END IF

    RETURN errNo, actionNo, l_br_uk.*
END FUNCTION
{</BLOCK>} --fct.action_zm_offerstastus

{<BLOCK Name="fct.action_zm_stakeholder">}
#+ Launch the module
#+
#+
#+ @return None
FUNCTION RFSManagerform_ui_action_zm_stakeholder()
    DEFINE l_uiSettings UISettings_Type
    ...
    DEFINE errNo INTEGER
    DEFINE actionNo SMALLINT
    {<POINT Name="fct.action_zm_stakeholder.define">} {</POINT>}
    ...
    LET l_br_uk.rfs_stakeholderId = IIF(m_record1_keyIndex > 0, m_record1_arrRecList[m_record1_keyIndex].rfs_stakeholderId, NULL)
    INITIALIZE l_whereRelation TO NULL

    {<POINT Name="fct.action_zm_stakeholder.init">} {</POINT>}
    IF RFSManagerform_events.m_DlgEvent__action_zm_stakeholder_BeforeOpeningTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_stakeholder_BeforeOpeningTheForm(m_state)
    END IF

    CALL StakeholderZoom_ui_uiOpenFormByKey(l_whereRelation, l_uiSettings.*, l_br_uk.*) RETURNING errNo, actionNo, l_br_uk.*

    IF RFSManagerform_events.m_DlgEvent__action_zm_stakeholder_AfterClosingTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_stakeholder_AfterClosingTheForm(m_state)
    END IF

    RETURN errNo, actionNo, l_br_uk.*
END FUNCTION
{</BLOCK>} --fct.action_zm_stakeholder

{<BLOCK Name="fct.action_zm_client">}
#+ Launch the module
#+
#+
#+ @return None
FUNCTION RFSManagerform_ui_action_zm_client()
    DEFINE l_uiSettings UISettings_Type
    ...
    DEFINE errNo INTEGER
    DEFINE actionNo SMALLINT
    {<POINT Name="fct.action_zm_client.define">} {</POINT>}
    ...
    LET l_br_uk.rfs_clientid = IIF(m_record1_keyIndex > 0, m_record1_arrRecList[m_record1_keyIndex].rfs_clientid, NULL)
    INITIALIZE l_whereRelation TO NULL

    {<POINT Name="fct.action_zm_client.init">} {</POINT>}
    IF RFSManagerform_events.m_DlgEvent__action_zm_client_BeforeOpeningTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_client_BeforeOpeningTheForm(m_state)
    END IF

    CALL ClientZoom_ui_uiOpenFormByKey(l_whereRelation, l_uiSettings.*, l_br_uk.*) RETURNING errNo, actionNo, l_br_uk.*

    IF RFSManagerform_events.m_DlgEvent__action_zm_client_AfterClosingTheForm IS NOT NULL THEN
        CALL RFSManagerform_events.m_DlgEvent__action_zm_client_AfterClosingTheForm(m_state)
    END IF

    RETURN errNo, actionNo, l_br_uk.*
END FUNCTION
{</BLOCK>} --fct.action_zm_client
...
FUNCTION RFSManagerform_ui_validateCRUDOperationWrapper(p_dialog, p_subdialog_name, p_state, p_action)
    ...
    CASE p_subdialog_name
        WHEN "record1"
            CALL RFSManagerform_uidialog.RFSManagerform_uidialog_record1_validateCRUDOperation(p_dialog, p_state, p_action)
                RETURNING l_internalAction
    END CASE
