...
#+
PUBLIC FUNCTION PayTermZoom_uidialogdata_record1_clear()
...
PUBLIC FUNCTION PayTermZoom_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL PayTermZoom_uidialogdata_record1_clear()
    CALL PayTermZoom_uidata.PayTermZoom_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION PayTermZoom_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL PayTermZoom_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL PayTermZoom_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION PayTermZoom_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL PayTermZoom_uidata.PayTermZoom_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].paymentterms_paytermid) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION PayTermZoom_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION PayTermZoom_uidialogdata_record1_seek(p_record1_br_uk)
