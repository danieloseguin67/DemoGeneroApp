...
#+ @return p_arr_br_uk
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_getKeys(p_arr_br_uk, p_where, p_detailList)
    ...
    IF ClientTypeConfigForm_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL ClientTypeConfigForm_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
            RETURNING l_sqlQuery
    END IF

    WHENEVER ERROR CONTINUE
        DECLARE getKeys_cursqlsid_ClientTypeConfigForm_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getKeys_cursqlsid_ClientTypeConfigForm_record1 INTO l_rec_data.*
            LET p_arr_br_uk[p_arr_br_uk.getLength() + 1].* = l_rec_data.*
        END FOREACH
        FREE getKeys_cursqlsid_ClientTypeConfigForm_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_uk.clear()
    END IF
...
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    ...
    WHENEVER ERROR CONTINUE
        ...
        IF ClientTypeConfigForm_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL ClientTypeConfigForm_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
...
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_insertRow(p_rec_br)
    DEFINE p_rec_br record1_br_type
    DEFINE dataInsert RECORD LIKE clienttypes.*
    ...
    CALL consultcompanion_dbxdata.consultcompanion_dbxdata_clienttypes_setDefaultValuesFromDBSchema() RETURNING dataInsert.*
    --Set local data with form field values
    LET dataInsert.clienttypeid = p_rec_br.clienttypes_clienttypeid
    LET dataInsert.clientypename = p_rec_br.clienttypes_clientypename
    {<POINT Name="fct.record1_insertRow.init">} {</POINT>}

    IF ClientTypeConfigForm_events.m_DataEvent_record1_BeforeInsertRow IS NOT NULL THEN
        CALL ClientTypeConfigForm_events.m_DataEvent_record1_BeforeInsertRow(dataInsert.*)
            RETURNING errNo, errMsg, dataInsert.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database insert row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_clienttypes_PK_clientTypes_1_insertRowByKey(dataInsert.*) RETURNING errNo, errMsg, ret.clienttypes_clienttypeid
        IF errNo == ERROR_SUCCESS THEN
            IF ClientTypeConfigForm_events.m_DataEvent_record1_AfterInsertRow IS NOT NULL THEN
                CALL ClientTypeConfigForm_events.m_DataEvent_record1_AfterInsertRow(errNo, errMsg, ret.*)
...
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_updateRow(p_rec_br, p_dataT0)
    DEFINE p_rec_br record1_br_type
    DEFINE p_dataT0 RECORD LIKE clienttypes.*
    DEFINE l_dataT1 RECORD LIKE clienttypes.*
    ...
    --Set local data with form field values
    LET l_dataT1.clienttypeid = p_rec_br.clienttypes_clienttypeid
    LET l_dataT1.clientypename = p_rec_br.clienttypes_clientypename
    --Set local Business Record Key data
    LET l_rec_br_uk.clienttypes_clienttypeid = p_dataT0.clienttypeid
    {<POINT Name="fct.record1_updateRow.init">} {</POINT>}

    IF ClientTypeConfigForm_events.m_DataEvent_record1_BeforeUpdateRow IS NOT NULL THEN
        CALL ClientTypeConfigForm_events.m_DataEvent_record1_BeforeUpdateRow(p_dataT0.*, l_dataT1.*)
            RETURNING errNo, errMsg, p_dataT0.*, l_dataT1.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database update row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_clienttypes_PK_clientTypes_1_updateRowByKey(p_dataT0.*, l_dataT1.*) RETURNING errNo, errMsg
        IF errNo == ERROR_SUCCESS THEN
            IF ClientTypeConfigForm_events.m_DataEvent_record1_AfterUpdateRow IS NOT NULL THEN
                CALL ClientTypeConfigForm_events.m_DataEvent_record1_AfterUpdateRow(errNo, errMsg, l_rec_br_uk.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_deleteRowWithConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE clienttypes.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    IF ClientTypeConfigForm_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess IS NOT NULL THEN
        CALL ClientTypeConfigForm_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess(p_dataT0.*)
            RETURNING errNo, errMsg, p_dataT0.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database update row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_clienttypes_PK_clientTypes_1_deleteRowByKeyWithConcurrentAccess(p_dataT0.*) RETURNING errNo
        IF errNo == ERROR_SUCCESS THEN
            IF ClientTypeConfigForm_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess IS NOT NULL THEN
                CALL ClientTypeConfigForm_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess(errNo, errMsg, p_dataT0.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
#+ @returnType INTEGER, STRING
#+ @return     ERROR_SUCCESS|ERROR_FAILURE|ERROR_CONCURRENT_ACCESS_NOTFOUND|ERROR_CONCURRENT_ACCESS_FAILURE, error message
PUBLIC FUNCTION ClientTypeConfigForm_uidata_record1_checkRowConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE clienttypes.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    CALL consultcompanion_dbxdata.consultcompanion_dbxdata_clienttypes_PK_clientTypes_1_checkRowByKeyWithConcurrentAccess(p_dataT0.*, FALSE) RETURNING errNo
    CASE errNo
        WHEN ERROR_SUCCESS
        WHEN ERROR_CONCURRENT_ACCESS_FAILURE
            LET errMsg = C_TXT_UPDATE_OVERWRITE
