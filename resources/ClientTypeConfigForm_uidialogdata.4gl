...
#+
PUBLIC FUNCTION ClientTypeConfigForm_uidialogdata_record1_clear()
...
PUBLIC FUNCTION ClientTypeConfigForm_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL ClientTypeConfigForm_uidialogdata_record1_clear()
    CALL ClientTypeConfigForm_uidata.ClientTypeConfigForm_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION ClientTypeConfigForm_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL ClientTypeConfigForm_uidialogdata_record1_fetchRow() RETURNING errNo
...
#+
FUNCTION ClientTypeConfigForm_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL ClientTypeConfigForm_uidata.ClientTypeConfigForm_uidata_record1_getDataByKey(m_record1_arrKeyRec[m_record1_keyIndex].clienttypes_clienttypeid) RETURNING errNo, m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION ClientTypeConfigForm_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION ClientTypeConfigForm_uidialogdata_record1_setDefaultValues(p_dialog)
    ...
    IF ClientTypeConfigForm_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL ClientTypeConfigForm_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecGrid[1].*)
...
PUBLIC FUNCTION ClientTypeConfigForm_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.clienttypes_clienttypeid == m_record1_arrKeyRec[m_record1_keyIndex].clienttypes_clienttypeid THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.clienttypes_clienttypeid == m_record1_arrKeyRec[i].clienttypes_clienttypeid THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
