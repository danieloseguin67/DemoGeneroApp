...
#+
PUBLIC FUNCTION RFSManagerform_uidialogdata_record1_clear()
...
PUBLIC FUNCTION RFSManagerform_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL RFSManagerform_uidialogdata_record1_clear()
    CALL RFSManagerform_uidata.RFSManagerform_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION RFSManagerform_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL RFSManagerform_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL RFSManagerform_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION RFSManagerform_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL RFSManagerform_uidata.RFSManagerform_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].rfs_rfsId) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION RFSManagerform_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION RFSManagerform_uidialogdata_record1_setDefaultValues(p_dialog)
    DEFINE p_dialog ui.Dialog
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_setDefaultValues.define">} {</POINT>}

    {<POINT Name="fct.record1_setDefaultValues.init">} {</POINT>}
    IF RFSManagerform_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL RFSManagerform_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecList[m_record1_keyIndex].*)
...
PUBLIC FUNCTION RFSManagerform_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF p_record1_br_uk.rfs_rfsId IS NULL THEN
        LET idxReturn = 0
    END IF

    --If the seeking values match the current row, then no need to seek and return the current index
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.rfs_rfsId == m_record1_arrRecList[m_record1_keyIndex].rfs_rfsId THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.rfs_rfsId == m_record1_arrRecList[i].rfs_rfsId THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
