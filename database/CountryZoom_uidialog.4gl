...
DIALOG CountryZoom_uidialog_record1_uiDisplay()
    ...
    DISPLAY ARRAY m_record1_arrRecList TO record1.*
        ...
        BEFORE DISPLAY
            ...
            CALL CountryZoom_uidialog_record1_setActionStates(DIALOG, C_MODE_DISPLAY)
            ...
            IF CountryZoom_events.m_DlgEvent_record1_BeforeDisplay IS NOT NULL THEN
                CALL CountryZoom_events.m_DlgEvent_record1_BeforeDisplay(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
        ...
        BEFORE ROW
            ...
            IF (m_record1_keyIndex != DIALOG.getCurrentRow("record1") OR m_prevAction == C_ACTION_CANCEL) THEN
                ...
                CALL CountryZoom_uidialogdata_record1_fetchDetails()
            END IF
            CALL CountryZoom_uidialog_setAllCurrentRow(DIALOG)
            IF NOT libdbapp_isMobile() THEN
                MESSAGE m_record1_keyIndex, "/", m_record1_arrRecList.getLength()
            END IF

            INITIALIZE CountryZoom_events.m_DlgCtrlInstruction TO NULL
            IF CountryZoom_events.m_DlgEvent_record1_BeforeRow IS NOT NULL THEN
                CALL CountryZoom_events.m_DlgEvent_record1_BeforeRow(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
                    RETURNING CountryZoom_events.m_DlgCtrlInstruction
            END IF
        ...
        AFTER DISPLAY
        INITIALIZE CountryZoom_events.m_DlgCtrlInstruction TO NULL
        IF CountryZoom_events.m_DlgEvent_record1_AfterDisplay IS NOT NULL THEN
            CALL CountryZoom_events.m_DlgEvent_record1_AfterDisplay(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
                RETURNING CountryZoom_events.m_DlgCtrlInstruction
        END IF
        ...
        AFTER ROW
        INITIALIZE CountryZoom_events.m_DlgCtrlInstruction TO NULL
        IF CountryZoom_events.m_DlgEvent_record1_AfterRow IS NOT NULL THEN
            CALL CountryZoom_events.m_DlgEvent_record1_AfterRow(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
                RETURNING CountryZoom_events.m_DlgCtrlInstruction
        END IF
...
DIALOG CountryZoom_uidialog_record1_uiConstruct()
    ...
    CONSTRUCT m_record1_where
        ...
        BEFORE CONSTRUCT
        INITIALIZE CountryZoom_events.m_DlgCtrlInstruction TO NULL
        IF CountryZoom_events.m_DlgEvent_record1_BeforeConstruct IS NOT NULL THEN
            CALL CountryZoom_events.m_DlgEvent_record1_BeforeConstruct(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
                RETURNING CountryZoom_events.m_DlgCtrlInstruction
        END IF
        ...
        AFTER CONSTRUCT
        INITIALIZE CountryZoom_events.m_DlgCtrlInstruction TO NULL
        IF CountryZoom_events.m_DlgEvent_record1_AfterConstruct IS NOT NULL THEN
            CALL CountryZoom_events.m_DlgEvent_record1_AfterConstruct(DIALOG, m_state, CountryZoom_events.m_DlgCtrlInstruction)
                RETURNING CountryZoom_events.m_DlgCtrlInstruction
        END IF
...
FUNCTION CountryZoom_uidialog_record1_validateCRUDOperation(p_dialog, p_state, p_action)
    ...
    IF l_internalAction != C_INTERNAL_ACTION_NEXT_FIELD_CURRENT AND l_internalAction != C_INTERNAL_EXIT_DIALOG THEN
        ...
        IF l_internalAction != C_INTERNAL_ACTION_NO_SYNC THEN
            CALL CountryZoom_uidialog_record1_synchronizeUI(p_dialog, p_state, l_internalAction, l_rec_br_uk.*) RETURNING m_copy_record1.*
        END IF
...
FUNCTION CountryZoom_uidialog_record1_synchronizeUI(p_dialog, p_state, p_internalAction, p_rec_br_uk)
    DEFINE p_dialog ui.Dialog
    DEFINE p_state SMALLINT
    ...
    WHILE l_continue
        ...
        CASE l_internalAction
            ...
            WHEN C_INTERNAL_ACTION_REFRESH_DATA
                CALL CountryZoom_uidialogdata_record1_fetchRowAndDetails() RETURNING errNo --Done to update the m_record1_arrRecDB
            ...
            WHEN C_INTERNAL_ACTION_REFRESH_CURRENT_ROW
                CALL CountryZoom_uidialogdata_record1_fetchRowAndDetails() RETURNING errNo
                IF errNo == ERROR_NOTFOUND THEN
                    LET l_internalAction = C_INTERNAL_ACTION_DELETE_DATA
                    LET l_continue = TRUE
                    CONTINUE WHILE
    ...
    CALL CountryZoom_uidialog_record1_setActionStates(p_dialog, p_state)
    IF NOT libdbapp_isMobile() THEN
        MESSAGE m_record1_keyIndex, "/", m_record1_arrRecList.getLength()
    END IF
...
FUNCTION CountryZoom_uidialog_setActionStates(p_dialog, p_mode)
    DEFINE p_dialog ui.Dialog
    ...
    IF CountryZoom_events.m_DlgEvent_OnActionStatesChange IS NOT NULL THEN
        CALL CountryZoom_events.m_DlgEvent_OnActionStatesChange(p_dialog, p_mode)
    END IF
END FUNCTION
...
#+ @param p_dialog      The current DIALOG
#+ @param p_mode        The current mode
FUNCTION CountryZoom_uidialog_record1_setActionStates(p_dialog, p_mode)
    DEFINE p_dialog ui.Dialog
    DEFINE p_mode SMALLINT
    {<POINT Name="fct.record1_setActionStates.define">} {</POINT>}

    --Sets the state of the DIALOG actions
    CALL CountryZoom_uidialog_setActionStates(p_dialog, p_mode)
    ...
    IF CountryZoom_events.m_DlgEvent_record1_OnActionStatesChange IS NOT NULL THEN
        CALL CountryZoom_events.m_DlgEvent_record1_OnActionStatesChange(p_dialog, p_mode)
    END IF
END FUNCTION
...
#+
#+ @param p_dialog The current DIALOG
PUBLIC FUNCTION CountryZoom_uidialog_setAllCurrentRow(p_dialog)
    DEFINE p_dialog ui.Dialog
