...
#+ @returnType INTEGER, record1_br_type, country.*
#+ @return Error number, 0-success, other-error
#+ @return p_arr_br_fields, p_arr_db_fields
PUBLIC FUNCTION CountryZoom_uidata_record1_getDataArray(p_arr_br_fields, p_arr_db_fields, p_where, p_detailList)
    --Declaration of parameters
    DEFINE p_arr_br_fields DYNAMIC ARRAY OF record1_br_type
    DEFINE p_arr_db_fields DYNAMIC ARRAY OF RECORD LIKE country.*
    ...
    IF CountryZoom_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL CountryZoom_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
    ...
    WHENEVER ERROR CONTINUE
        DECLARE getDataArray_cursqlsid_CountryZoom_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getDataArray_cursqlsid_CountryZoom_record1 INTO
                p_arr_br_fields[i].country_countryid,
                p_arr_br_fields[i].country_countryname,
                p_arr_db_fields[i].*

            IF CountryZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
                CALL CountryZoom_events.m_DataEvent_record1_OnComputedFields(p_arr_br_fields[i])
            END IF
        ...
        FREE getDataArray_cursqlsid_CountryZoom_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_fields.clear()
        CALL p_arr_db_fields.clear()
...
PUBLIC FUNCTION CountryZoom_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    DEFINE p_rec_br_uk record1_br_uk_type
    --Declaration of local types, variables, constants
    DEFINE l_rec_BRdata record1_br_type
    DEFINE l_rec_DBdata RECORD LIKE country.*
    ...
    WHENEVER ERROR CONTINUE
        SELECT @country.countryid,
            @country.countryname,
            country.*
        INTO l_rec_BRdata.country_countryid,
            l_rec_BRdata.country_countryname,
            l_rec_DBdata.*
        FROM country
        WHERE @country.countryid = p_rec_br_uk.country_countryid
        IF SQLCA.SQLCODE != 0 THEN
        ...
        IF CountryZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL CountryZoom_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
