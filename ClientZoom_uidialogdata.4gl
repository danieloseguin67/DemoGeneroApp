...
#+
PUBLIC FUNCTION ClientZoom_uidialogdata_record1_clear()
...
PUBLIC FUNCTION ClientZoom_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL ClientZoom_uidialogdata_record1_clear()
    CALL ClientZoom_uidata.ClientZoom_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION ClientZoom_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL ClientZoom_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL ClientZoom_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION ClientZoom_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL ClientZoom_uidata.ClientZoom_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].clients_clientid) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION ClientZoom_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION ClientZoom_uidialogdata_record1_seek(p_record1_br_uk)
