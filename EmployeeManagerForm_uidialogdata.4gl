...
#+
PUBLIC FUNCTION EmployeeManagerForm_uidialogdata_record1_clear()
...
PUBLIC FUNCTION EmployeeManagerForm_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL EmployeeManagerForm_uidialogdata_record1_clear()
    CALL EmployeeManagerForm_uidata.EmployeeManagerForm_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION EmployeeManagerForm_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL EmployeeManagerForm_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL EmployeeManagerForm_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION EmployeeManagerForm_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL EmployeeManagerForm_uidata.EmployeeManagerForm_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].employees_employeeid) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION EmployeeManagerForm_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION EmployeeManagerForm_uidialogdata_record1_setDefaultValues(p_dialog)
    DEFINE p_dialog ui.Dialog
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_setDefaultValues.define">} {</POINT>}

    {<POINT Name="fct.record1_setDefaultValues.init">} {</POINT>}
    IF EmployeeManagerForm_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL EmployeeManagerForm_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecList[m_record1_keyIndex].*)
...
PUBLIC FUNCTION EmployeeManagerForm_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF p_record1_br_uk.employees_employeeid IS NULL THEN
        LET idxReturn = 0
    END IF

    --If the seeking values match the current row, then no need to seek and return the current index
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.employees_employeeid == m_record1_arrRecList[m_record1_keyIndex].employees_employeeid THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.employees_employeeid == m_record1_arrRecList[i].employees_employeeid THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
