...
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_getDataArray(p_arr_br_fields, p_arr_db_fields, p_where, p_detailList)
    ...
    IF ClientManagerForm2_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL ClientManagerForm2_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
    ...
    WHENEVER ERROR CONTINUE
        DECLARE getDataArray_cursqlsid_ClientManagerForm2_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getDataArray_cursqlsid_ClientManagerForm2_record1 INTO
            ...
            IF ClientManagerForm2_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
                CALL ClientManagerForm2_events.m_DataEvent_record1_OnComputedFields(p_arr_br_fields[i])
            END IF
        ...
        FREE getDataArray_cursqlsid_ClientManagerForm2_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_fields.clear()
        CALL p_arr_db_fields.clear()
...
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    ...
    WHENEVER ERROR CONTINUE
        ...
        IF ClientManagerForm2_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL ClientManagerForm2_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
...
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_insertRow(p_rec_br)
    ...
    IF ClientManagerForm2_events.m_DataEvent_record1_BeforeInsertRow IS NOT NULL THEN
        CALL ClientManagerForm2_events.m_DataEvent_record1_BeforeInsertRow(dataInsert.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF ClientManagerForm2_events.m_DataEvent_record1_AfterInsertRow IS NOT NULL THEN
                CALL ClientManagerForm2_events.m_DataEvent_record1_AfterInsertRow(errNo, errMsg, ret.*)
...
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_updateRow(p_rec_br, p_dataT0)
    DEFINE p_rec_br record1_br_type
    ...
    IF ClientManagerForm2_events.m_DataEvent_record1_BeforeUpdateRow IS NOT NULL THEN
        CALL ClientManagerForm2_events.m_DataEvent_record1_BeforeUpdateRow(p_dataT0.*, l_dataT1.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF ClientManagerForm2_events.m_DataEvent_record1_AfterUpdateRow IS NOT NULL THEN
                CALL ClientManagerForm2_events.m_DataEvent_record1_AfterUpdateRow(errNo, errMsg, l_rec_br_uk.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_deleteRowWithConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE clients.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    IF ClientManagerForm2_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess IS NOT NULL THEN
        CALL ClientManagerForm2_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess(p_dataT0.*)
    ...
    IF errNo == ERROR_SUCCESS THEN
        ...
        IF errNo == ERROR_SUCCESS THEN
            IF ClientManagerForm2_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess IS NOT NULL THEN
                CALL ClientManagerForm2_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess(errNo, errMsg, p_dataT0.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
#+ @returnType INTEGER, STRING
#+ @return     ERROR_SUCCESS|ERROR_FAILURE|ERROR_CONCURRENT_ACCESS_NOTFOUND|ERROR_CONCURRENT_ACCESS_FAILURE, error message
PUBLIC FUNCTION ClientManagerForm2_uidata_record1_checkRowConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE clients.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
