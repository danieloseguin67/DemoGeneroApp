...
#+
PUBLIC FUNCTION PreferencesForm_uidialogdata_record1_clear()
...
PUBLIC FUNCTION PreferencesForm_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL PreferencesForm_uidialogdata_record1_clear()
    CALL PreferencesForm_uidata.PreferencesForm_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION PreferencesForm_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL PreferencesForm_uidialogdata_record1_fetchRow() RETURNING errNo
...
#+
FUNCTION PreferencesForm_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL PreferencesForm_uidata.PreferencesForm_uidata_record1_getDataByKey(m_record1_arrKeyRec[m_record1_keyIndex].preferences_seqid) RETURNING errNo, m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION PreferencesForm_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION PreferencesForm_uidialogdata_record1_setDefaultValues(p_dialog)
    ...
    IF PreferencesForm_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL PreferencesForm_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecGrid[1].*)
...
PUBLIC FUNCTION PreferencesForm_uidialogdata_record1_seek(p_record1_br_uk)
