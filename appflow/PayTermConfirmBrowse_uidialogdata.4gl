...
#+
PUBLIC FUNCTION PayTermConfirmBrowse_uidialogdata_record1_clear()
...
PUBLIC FUNCTION PayTermConfirmBrowse_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL PayTermConfirmBrowse_uidialogdata_record1_clear()
    CALL PayTermConfirmBrowse_uidata.PayTermConfirmBrowse_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION PayTermConfirmBrowse_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL PayTermConfirmBrowse_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL PayTermConfirmBrowse_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION PayTermConfirmBrowse_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL PayTermConfirmBrowse_uidata.PayTermConfirmBrowse_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].paymentterms_paytermid) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION PayTermConfirmBrowse_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION PayTermConfirmBrowse_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.paymentterms_paytermid == m_record1_arrRecList[m_record1_keyIndex].paymentterms_paytermid THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.paymentterms_paytermid == m_record1_arrRecList[i].paymentterms_paytermid THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
