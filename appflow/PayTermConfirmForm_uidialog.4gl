{<CODEFILE Path="PayTermConfirmForm.code" Hash="1B2M2Y8AsgTpgAmY7PhCfg==" />}
...
IMPORT FGL PayTermConfirmForm_common
IMPORT FGL PayTermConfirmForm
IMPORT FGL PayTermConfirmForm_events
IMPORT FGL PayTermConfirmForm_uidata
IMPORT FGL PayTermConfirmForm_uidialogdata
...
DIALOG PayTermConfirmForm_uidialog_record1_uiDisplay()
    ...
    DISPLAY ARRAY m_record1_arrRecGrid TO record1.* ATTRIBUTES(COUNT=IIF(m_record1_arrKeyRec.getLength() <= 0, 1, m_record1_arrKeyRec.getLength()))
        BEFORE DISPLAY
            ...
            CALL PayTermConfirmForm_uidialog_record1_setActionStates(DIALOG, C_MODE_DISPLAY)
            ...
            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_BeforeDisplay IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_BeforeDisplay(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF

            CASE PayTermConfirmForm_events.m_DlgCtrlInstruction
        ...
        BEFORE ROW
            ...
            IF (m_record1_keyIndex != DIALOG.getCurrentRow("record1") OR m_prevAction == C_ACTION_CANCEL) THEN
                ...
                CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(DIALOG, m_state, C_ACTION_REFRESH_CURRENT_ROW) RETURNING l_internalAction
            END IF
            CALL PayTermConfirmForm_uidialog_setAllCurrentRow(DIALOG)
            IF m_record1_arrKeyRec.getLength() > 1 THEN
                MESSAGE m_record1_keyIndex, "/", m_record1_arrKeyRec.getLength()
            END IF

            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_BeforeRow IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_BeforeRow(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION append
            ...
            LET PayTermConfirmForm_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF PayTermConfirmForm_events.m_DlgEvent_record1_OnActionAppend IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_OnActionAppend(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION update
            ...
            LET PayTermConfirmForm_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF PayTermConfirmForm_events.m_DlgEvent_record1_OnActionUpdate IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_OnActionUpdate(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION delete --DISPLAY (record1)
            ...
            CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(DIALOG, m_state, C_ACTION_DELETE) RETURNING l_internalAction
            ...
            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_OnActionDelete IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_OnActionDelete(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        AFTER DISPLAY
        INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
        IF PayTermConfirmForm_events.m_DlgEvent_record1_AfterDisplay IS NOT NULL THEN
            CALL PayTermConfirmForm_events.m_DlgEvent_record1_AfterDisplay(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
        END IF
        ...
        AFTER ROW
        INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
        IF PayTermConfirmForm_events.m_DlgEvent_record1_AfterRow IS NOT NULL THEN
            CALL PayTermConfirmForm_events.m_DlgEvent_record1_AfterRow(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
        END IF
...
DIALOG PayTermConfirmForm_uidialog_record1_uiInput()
    DEFINE l_internalAction SMALLINT
    DEFINE errNo INTEGER

    {<POINT Name="dlg.record1.uiInput.define">} {</POINT>}

    INPUT m_record1_arrRecGrid[1].paymentterms_paytermid,
        m_record1_arrRecGrid[1].paymentterms_paytermname,
        m_record1_arrRecGrid[1].paymentterms_paytermdays
    FROM  paymentterms.paytermid,
        paymentterms.paytermname,
        paymentterms.paytermdays
    ATTRIBUTES(WITHOUT DEFAULTS = TRUE)
        BEFORE INPUT
            ...
            IF m_navigation_direction != 0 THEN
                CALL PayTermConfirmForm_uidialog_record1_navigate(DIALOG,C_MODE_MODIFY,m_navigation_direction)
            ...
            IF initializeUI THEN
                CALL PayTermConfirmForm_uidialog_record1_setFieldActive(DIALOG, m_state)
            ...
            IF m_state == C_MODE_ADD THEN
                CALL PayTermConfirmForm_uidialog_record1_navigate(DIALOG, C_MODE_ADD, 3)
                CALL PayTermConfirmForm_uidialogdata_record1_setDefaultValues(DIALOG)
                CALL PayTermConfirmForm_uidialogdata_record1_fetchDetails()
            END IF
            CALL PayTermConfirmForm_uidialog_record1_setFieldActive(DIALOG, m_state)
            CALL PayTermConfirmForm_uidialog_record1_setActionStates(DIALOG, m_state)
            CALL PayTermConfirmForm_uidialog_setAllCurrentRow(DIALOG)
            IF m_record1_arrKeyRec.getLength() > 1 THEN
                MESSAGE m_record1_keyIndex, "/", m_record1_arrKeyRec.getLength()
            END IF
            ...
            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_BeforeInput IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_BeforeInput(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        AFTER INPUT
            ...
            CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(DIALOG, m_state, m_actionNo) RETURNING l_internalAction
            CASE l_internalAction
                WHEN C_INTERNAL_ACTION_NEXT_FIELD_CURRENT --We remain in input mode
            ...
            IF m_dataProcessing THEN --Skip the DVM validation because a data processing is in progress in an other subdialog
                LET PayTermConfirmForm_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            ELSE
                INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            END IF
            IF PayTermConfirmForm_events.m_DlgEvent_record1_AfterInput IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_AfterInput(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION delete --MODIFY (record1)
            ...
            CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(DIALOG, C_MODE_MODIFY, C_ACTION_DELETE) RETURNING l_internalAction
            CASE l_internalAction
                WHEN C_INTERNAL_ACTION_NEXT_FIELD_CURRENT --We remain in input mode
                    CALL DIALOG.nextField(FGL_DIALOG_GETFIELDNAME())
                    LET m_dataProcessing = FALSE
        ...
        BEFORE FIELD paymentterms.paytermid
            {<POINT Name="dlg.record1.uiInput.beforeField.paymentterms.paytermid">} {</POINT>}
            ...
            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_BeforeField IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_BeforeField(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction, "paymentterms.paytermid")
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        AFTER FIELD paymentterms.paytermid
            {<POINT Name="dlg.record1.uiInput.afterField.paymentterms.paytermid">} {</POINT>}
            ...
            INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
            IF PayTermConfirmForm_events.m_DlgEvent_record1_AfterField IS NOT NULL THEN
                CALL PayTermConfirmForm_events.m_DlgEvent_record1_AfterField(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction, "paymentterms.paytermid")
                    RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION dialogtouched --INPUT (record1)
            ...
            IF m_state == C_MODE_MODIFY THEN
                CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(DIALOG, C_MODE_MODIFY, C_ACTION_DIALOGTOUCHED) RETURNING l_internalAction
                CASE l_internalAction
                    WHEN C_INTERNAL_ACTION_NEXT_FIELD_CURRENT --We remain in input mode
                        CALL DIALOG.nextField(FGL_DIALOG_GETFIELDNAME())
                        CONTINUE DIALOG
...
DIALOG PayTermConfirmForm_uidialog_record1_uiConstruct()

    {<POINT Name="dlg.record1.uiConstruct.define">} {</POINT>}

    CONSTRUCT m_record1_where
                 ON paymentterms.paytermid,
                    paymentterms.paytermname,
                    paymentterms.paytermdays
             FROM paymentterms.paytermid,
                    paymentterms.paytermname,
                    paymentterms.paytermdays
        BEFORE FIELD paymentterms.paytermid
            {<POINT Name="dlg.record1.uiConstruct.beforeField.paymentterms.paytermid">} {</POINT>}
            CALL DIALOG.setActionActive('zoom', TRUE)
            IF libdbapp_isMobile() THEN
                CALL DIALOG.setActionHidden('zoom', FALSE)
            END IF
        ...
        AFTER FIELD paymentterms.paytermid
            {<POINT Name="dlg.record1.uiConstruct.afterField.paymentterms.paytermid">} {</POINT>}
            CALL DIALOG.setActionActive('zoom', FALSE)
            IF libdbapp_isMobile() THEN
                CALL DIALOG.setActionHidden('zoom', TRUE)
            END IF
        ...
        BEFORE CONSTRUCT
        INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
        IF PayTermConfirmForm_events.m_DlgEvent_record1_BeforeConstruct IS NOT NULL THEN
            CALL PayTermConfirmForm_events.m_DlgEvent_record1_BeforeConstruct(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
        END IF
        ...
        AFTER CONSTRUCT
        INITIALIZE PayTermConfirmForm_events.m_DlgCtrlInstruction TO NULL
        IF PayTermConfirmForm_events.m_DlgEvent_record1_AfterConstruct IS NOT NULL THEN
            CALL PayTermConfirmForm_events.m_DlgEvent_record1_AfterConstruct(DIALOG, m_state, PayTermConfirmForm_events.m_DlgCtrlInstruction)
                RETURNING PayTermConfirmForm_events.m_DlgCtrlInstruction
        END IF
...
FUNCTION PayTermConfirmForm_uidialog_record1_validateCRUDOperation(p_dialog, p_state, p_action)
    ...
    IF l_internalAction == C_INTERNAL_ACTION_PROCESS_OPERATION THEN
        ...
        IF l_action != C_ACTION_DELETE AND l_action != C_ACTION_DIALOGTOUCHED THEN
            ...
            END IF
        END IF
        CALL PayTermConfirmForm_uidialog_record1_processCRUDOperation(l_action) RETURNING l_internalAction, l_rec_br_uk.*
    END IF
    IF l_internalAction != C_INTERNAL_ACTION_NEXT_FIELD_CURRENT AND l_internalAction != C_INTERNAL_EXIT_DIALOG THEN
        ...
        IF l_internalAction != C_INTERNAL_ACTION_NO_SYNC THEN
            CALL PayTermConfirmForm_uidialog_record1_synchronizeUI(p_dialog, p_state, l_internalAction, l_rec_br_uk.*) RETURNING m_copy_record1.*
        END IF
...
FUNCTION PayTermConfirmForm_uidialog_record1_processCRUDOperation(p_action)
    DEFINE p_action SMALLINT
    DEFINE l_internalAction SMALLINT
    DEFINE l_rec_br_uk record1_br_uk_type --key returned by INSERT operation
    DEFINE errNo INTEGER
    ...
    CASE p_action
        WHEN C_ACTION_APPEND --Insert the row in the database
            CALL PayTermConfirmForm_uidata.PayTermConfirmForm_uidata_record1_insertRow(m_record1_arrRecGrid[1].*) RETURNING errNo, errMsg, l_rec_br_uk.*
        WHEN C_ACTION_UPDATE --Update the row in the database
            CALL PayTermConfirmForm_uidata.PayTermConfirmForm_uidata_record1_updateRow(m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*) RETURNING errNo, errMsg
        WHEN C_ACTION_DIALOGTOUCHED --Check the row in the database
            CALL PayTermConfirmForm_uidata.PayTermConfirmForm_uidata_record1_checkRowConcurrentAccess(m_record1_arrRecDB[1].*) RETURNING errNo, errMsg

        WHEN C_ACTION_DELETE --Delete the row in the database
            CALL PayTermConfirmForm_uidata.PayTermConfirmForm_uidata_record1_deleteRowWithConcurrentAccess(m_record1_arrRecDB[1].*) RETURNING errNo, errMsg
        OTHERWISE
         -- unexpected error
    END CASE
...
FUNCTION PayTermConfirmForm_uidialog_record1_synchronizeUI(p_dialog, p_state, p_internalAction, p_rec_br_uk)
    DEFINE p_dialog ui.Dialog
    DEFINE p_state SMALLINT
        ...
        CASE l_internalAction
            WHEN C_INTERNAL_ACTION_SUCCESS
                IF p_state == C_MODE_ADD THEN
                    ...
                    LET m_record1_arrRecGrid[1].paymentterms_paytermid = p_rec_br_uk.paymentterms_paytermid --Refresh the inserted key value in the UI (may be a composite key) (Useful in the case of SERIALS which are known after the INSERT sql statement)
                END IF
                CALL PayTermConfirmForm_uidialogdata_record1_fetchRowAndDetails() RETURNING errNo --Done to update the m_record1_arrRecDB
            ...
            WHEN C_INTERNAL_ACTION_REFRESH_CURRENT_ROW
                CALL PayTermConfirmForm_uidialogdata_record1_fetchRowAndDetails() RETURNING errNo
                IF errNo == ERROR_NOTFOUND THEN
                    LET l_internalAction = C_INTERNAL_ACTION_DELETE_DATA
                    LET l_continue = TRUE
                    CONTINUE WHILE
            ...
            WHEN C_INTERNAL_ACTION_REFRESH_ALL_ROWS
                CALL PayTermConfirmForm_uidialogdata_record1_fetchAll(m_where, m_detailList)
            ...
            WHEN C_INTERNAL_ACTION_RESTORE_DATA
                ...
                IF p_state == C_MODE_ADD THEN
                    ...
                    CALL PayTermConfirmForm_uidialog_record1_validateCRUDOperation(p_dialog, p_state, C_ACTION_REFRESH_CURRENT_ROW) RETURNING l_internalAction
                END IF
    ...
    IF p_state != C_MODE_DISPLAY THEN
        ...
        CALL PayTermConfirmForm_uidialog_record1_setFieldActive(p_dialog, p_state)
    END IF
    IF p_state == C_MODE_DISPLAY THEN
        CALL p_dialog.setArrayLength("record1", m_record1_arrKeyRec.getLength())
    END IF

    CALL PayTermConfirmForm_uidialog_record1_setActionStates(p_dialog, p_state)
    IF m_record1_arrKeyRec.getLength() > 1 THEN
        MESSAGE m_record1_keyIndex, "/", m_record1_arrKeyRec.getLength()
    END IF
    LET m_copy_record1.* = m_record1_arrRecGrid[1].* --Keep a local copy
...
FUNCTION PayTermConfirmForm_uidialog_setActionStates(p_dialog, p_mode)
    DEFINE p_dialog ui.Dialog
    ...
    IF PayTermConfirmForm_events.m_DlgEvent_OnActionStatesChange IS NOT NULL THEN
        CALL PayTermConfirmForm_events.m_DlgEvent_OnActionStatesChange(p_dialog, p_mode)
    END IF
END FUNCTION
...
#+ @param p_dialog      The current DIALOG
#+ @param p_mode        The current mode
FUNCTION PayTermConfirmForm_uidialog_record1_setActionStates(p_dialog, p_mode)
    DEFINE p_dialog ui.Dialog
    DEFINE p_mode SMALLINT
    {<POINT Name="fct.record1_setActionStates.define">} {</POINT>}

    --Sets the state of the DIALOG actions
    CALL PayTermConfirmForm_uidialog_setActionStates(p_dialog, p_mode)
    ...
    IF PayTermConfirmForm_events.m_DlgEvent_record1_OnActionStatesChange IS NOT NULL THEN
        CALL PayTermConfirmForm_events.m_DlgEvent_record1_OnActionStatesChange(p_dialog, p_mode)
    END IF
END FUNCTION
...
PUBLIC FUNCTION PayTermConfirmForm_uidialog_record1_setFieldActive(p_dialog, p_state)
    DEFINE p_dialog ui.Dialog
    DEFINE p_state SMALLINT
    {<POINT Name="fct.record1_setFieldActive.define">} {</POINT>}
    {<POINT Name="fct.record1_setFieldActive.init">} {</POINT>}

    CALL p_dialog.setFieldActive("paymentterms.paytermid", p_state == C_MODE_ADD)

    {<POINT Name="fct.record1_setFieldActive.user">} {</POINT>}
    IF PayTermConfirmForm_events.m_DlgEvent_record1_OnFieldsActivation IS NOT NULL THEN
        CALL PayTermConfirmForm_events.m_DlgEvent_record1_OnFieldsActivation(p_dialog, p_state)
    END IF
END FUNCTION
...
#+                    3 to add a row
FUNCTION PayTermConfirmForm_uidialog_record1_navigate(p_dialog, p_state, p_direction)
    DEFINE p_dialog ui.Dialog
    DEFINE p_state SMALLINT
...
#+
#+ @param p_dialog The current DIALOG
PUBLIC FUNCTION PayTermConfirmForm_uidialog_setAllCurrentRow(p_dialog)
    DEFINE p_dialog ui.Dialog
