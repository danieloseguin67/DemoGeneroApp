...
#+ @return p_arr_br_uk
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_getKeys(p_arr_br_uk, p_where, p_detailList)
    ...
    LET l_sqlQuery = 'SELECT ', l_distinct
        , ' offerstatuses.offerstatusid'
        , l_from
        , p_where
        , ''
        , ' ORDER BY offerstatuses.offerstatusid ASC '
    {<POINT Name="fct.record1_getKeys.init">} {</POINT>}
    IF OfferStatusConfigForm_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL OfferStatusConfigForm_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
            RETURNING l_sqlQuery
    END IF

    WHENEVER ERROR CONTINUE
        DECLARE getKeys_cursqlsid_OfferStatusConfigForm_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getKeys_cursqlsid_OfferStatusConfigForm_record1 INTO l_rec_data.*
            LET p_arr_br_uk[p_arr_br_uk.getLength() + 1].* = l_rec_data.*
        END FOREACH
        FREE getKeys_cursqlsid_OfferStatusConfigForm_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_uk.clear()
    END IF
...
#+
#+ @returnType INTEGER, record1_br_type, offerstatuses.*
#+ @return Error number, 0-success, other-error
#+ @return Data for the given Business Record (BR) Unique Key (UK)
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    DEFINE p_rec_br_uk record1_br_uk_type
    --Declaration of local types, variables, constants
    DEFINE l_rec_BRdata record1_br_type
    DEFINE l_rec_DBdata RECORD LIKE offerstatuses.*
    {<POINT Name="fct.record1_getDataByKey.define">} {</POINT>}

    {<POINT Name="fct.record1_getDataByKey.init">} {</POINT>}
    WHENEVER ERROR CONTINUE
        SELECT @offerstatuses.offerstatusid,
            @offerstatuses.offerstatusname,
            offerstatuses.*
        INTO l_rec_BRdata.offerstatuses_offerstatusid,
            l_rec_BRdata.offerstatuses_offerstatusname,
            l_rec_DBdata.*
        FROM offerstatuses
        WHERE @offerstatuses.offerstatusid = p_rec_br_uk.offerstatuses_offerstatusid
        IF SQLCA.SQLCODE != 0 THEN
            INITIALIZE l_rec_BRdata.* TO NULL
            INITIALIZE l_rec_DBdata.* TO NULL
        END IF

        IF OfferStatusConfigForm_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL OfferStatusConfigForm_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
...
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_insertRow(p_rec_br)
    DEFINE p_rec_br record1_br_type
    DEFINE dataInsert RECORD LIKE offerstatuses.*
    ...
    CALL consultcompanion_dbxdata.consultcompanion_dbxdata_offerstatuses_setDefaultValuesFromDBSchema() RETURNING dataInsert.*
    --Set local data with form field values
    LET dataInsert.offerstatusid = p_rec_br.offerstatuses_offerstatusid
    LET dataInsert.offerstatusname = p_rec_br.offerstatuses_offerstatusname
    {<POINT Name="fct.record1_insertRow.init">} {</POINT>}

    IF OfferStatusConfigForm_events.m_DataEvent_record1_BeforeInsertRow IS NOT NULL THEN
        CALL OfferStatusConfigForm_events.m_DataEvent_record1_BeforeInsertRow(dataInsert.*)
            RETURNING errNo, errMsg, dataInsert.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database insert row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_offerstatuses_PK_offerStatuses_1_insertRowByKey(dataInsert.*) RETURNING errNo, errMsg, ret.offerstatuses_offerstatusid
        IF errNo == ERROR_SUCCESS THEN
            IF OfferStatusConfigForm_events.m_DataEvent_record1_AfterInsertRow IS NOT NULL THEN
                CALL OfferStatusConfigForm_events.m_DataEvent_record1_AfterInsertRow(errNo, errMsg, ret.*)
...
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_updateRow(p_rec_br, p_dataT0)
    DEFINE p_rec_br record1_br_type
    DEFINE p_dataT0 RECORD LIKE offerstatuses.*
    DEFINE l_dataT1 RECORD LIKE offerstatuses.*
    ...
    --Set local data with form field values
    LET l_dataT1.offerstatusid = p_rec_br.offerstatuses_offerstatusid
    LET l_dataT1.offerstatusname = p_rec_br.offerstatuses_offerstatusname
    --Set local Business Record Key data
    LET l_rec_br_uk.offerstatuses_offerstatusid = p_dataT0.offerstatusid
    {<POINT Name="fct.record1_updateRow.init">} {</POINT>}

    IF OfferStatusConfigForm_events.m_DataEvent_record1_BeforeUpdateRow IS NOT NULL THEN
        CALL OfferStatusConfigForm_events.m_DataEvent_record1_BeforeUpdateRow(p_dataT0.*, l_dataT1.*)
            RETURNING errNo, errMsg, p_dataT0.*, l_dataT1.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database update row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_offerstatuses_PK_offerStatuses_1_updateRowByKey(p_dataT0.*, l_dataT1.*) RETURNING errNo, errMsg
        IF errNo == ERROR_SUCCESS THEN
            IF OfferStatusConfigForm_events.m_DataEvent_record1_AfterUpdateRow IS NOT NULL THEN
                CALL OfferStatusConfigForm_events.m_DataEvent_record1_AfterUpdateRow(errNo, errMsg, l_rec_br_uk.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_deleteRowWithConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE offerstatuses.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    IF OfferStatusConfigForm_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess IS NOT NULL THEN
        CALL OfferStatusConfigForm_events.m_DataEvent_record1_BeforeDeleteRowWithConcurrentAccess(p_dataT0.*)
            RETURNING errNo, errMsg, p_dataT0.*
    END IF
    IF errNo == ERROR_SUCCESS THEN
        --Call the database update row
        CALL consultcompanion_dbxdata.consultcompanion_dbxdata_offerstatuses_PK_offerStatuses_1_deleteRowByKeyWithConcurrentAccess(p_dataT0.*) RETURNING errNo
        IF errNo == ERROR_SUCCESS THEN
            IF OfferStatusConfigForm_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess IS NOT NULL THEN
                CALL OfferStatusConfigForm_events.m_DataEvent_record1_AfterDeleteRowWithConcurrentAccess(errNo, errMsg, p_dataT0.*)
                    RETURNING errNo, errMsg
            END IF
        END IF
    END IF
...
#+ @returnType INTEGER, STRING
#+ @return     ERROR_SUCCESS|ERROR_FAILURE|ERROR_CONCURRENT_ACCESS_NOTFOUND|ERROR_CONCURRENT_ACCESS_FAILURE, error message
PUBLIC FUNCTION OfferStatusConfigForm_uidata_record1_checkRowConcurrentAccess(p_dataT0)
    DEFINE p_dataT0 RECORD LIKE offerstatuses.*
    DEFINE errNo INTEGER
    DEFINE errMsg STRING
    ...
    CALL consultcompanion_dbxdata.consultcompanion_dbxdata_offerstatuses_PK_offerStatuses_1_checkRowByKeyWithConcurrentAccess(p_dataT0.*, FALSE) RETURNING errNo
    CASE errNo
        WHEN ERROR_SUCCESS
        WHEN ERROR_CONCURRENT_ACCESS_FAILURE
            LET errMsg = C_TXT_UPDATE_OVERWRITE
