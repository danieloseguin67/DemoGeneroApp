...
#+
PUBLIC FUNCTION SerialNumConfigForm_uidialogdata_record1_clear()
...
PUBLIC FUNCTION SerialNumConfigForm_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL SerialNumConfigForm_uidialogdata_record1_clear()
    CALL SerialNumConfigForm_uidata.SerialNumConfigForm_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION SerialNumConfigForm_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL SerialNumConfigForm_uidialogdata_record1_fetchRow() RETURNING errNo
...
#+
FUNCTION SerialNumConfigForm_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL SerialNumConfigForm_uidata.SerialNumConfigForm_uidata_record1_getDataByKey(m_record1_arrKeyRec[m_record1_keyIndex].seqreg_sr_name) RETURNING errNo, m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION SerialNumConfigForm_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION SerialNumConfigForm_uidialogdata_record1_setDefaultValues(p_dialog)
    ...
    IF SerialNumConfigForm_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL SerialNumConfigForm_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecGrid[1].*)
...
PUBLIC FUNCTION SerialNumConfigForm_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF p_record1_br_uk.seqreg_sr_name IS NULL THEN
        LET idxReturn = 0
    END IF

    --If the seeking values match the current row, then no need to seek and return the current index
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.seqreg_sr_name == m_record1_arrKeyRec[m_record1_keyIndex].seqreg_sr_name THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.seqreg_sr_name == m_record1_arrKeyRec[i].seqreg_sr_name THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
