{<CODEFILE Path="EmployeeTypeBrowse.code" Hash="1B2M2Y8AsgTpgAmY7PhCfg==" />}
#+ User Interface
...
IMPORT FGL EmployeeTypeBrowse_common
IMPORT FGL EmployeeTypeBrowse
IMPORT FGL EmployeeTypeBrowse_events
IMPORT FGL EmployeeTypeBrowse_uidata
IMPORT FGL EmployeeTypeBrowse_uidialogdata
IMPORT FGL EmployeeTypeBrowse_uidialog
...
PUBLIC FUNCTION EmployeeTypeBrowse_ui_uiOpenFormByKey(p_whereRelation, p_uiSettings, p_br_uk)
    ...
    OPEN WINDOW w_EmployeeTypeBrowse WITH FORM 'EmployeeTypeBrowse' ATTRIBUTES(STYLE=NVL(l_windowStyle, "zoom_dbapp"))
    --Initialize UI settings with default form UI settings
    CALL EmployeeTypeBrowse_ui_initializeDefaultUISettings() RETURNING l_defaultUISettings.*
    ...
    IF EmployeeTypeBrowse_events.m_DlgEvent_OnOpenForm IS NOT NULL THEN
        CALL EmployeeTypeBrowse_events.m_DlgEvent_OnOpenForm(w.getForm())
    END IF
    ...
    IF m_uiSettings.openMode != C_MODE_SEARCH AND m_uiSettings.openMode != C_MODE_EMPTY THEN
        CALL EmployeeTypeBrowse_uidialogdata_record1_fetchAll(m_where, m_detailList)
        LET m_record1_keyIndex = EmployeeTypeBrowse_uidialogdata_record1_seek(p_br_uk.*)
    END IF
    --Call the UI automaton
    CALL EmployeeTypeBrowse_ui_uiAutomaton(m_uiSettings.*) RETURNING errNo, actionNo
    INITIALIZE l_record1_br_uk.* TO NULL
    IF m_record1_keyIndex > 0 THEN
        LET l_record1_br_uk.employeetypes_emptypeid = m_record1_arrRecList[m_record1_keyIndex].employeetypes_emptypeid
    END IF
    CLOSE WINDOW w_EmployeeTypeBrowse
...
PRIVATE FUNCTION EmployeeTypeBrowse_ui_uiAutomaton(p_uiSettings)
    DEFINE p_uiSettings UISettings_Type
    ...
    WHILE runningAutomaton
        CASE l_state
            ...
            WHEN C_MODE_DISPLAY
                ...
                CALL EmployeeTypeBrowse_ui_uiDisplay(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_SEARCH
                ...
                CALL EmployeeTypeBrowse_ui_uiConstruct(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_EMPTY
                ...
                CALL EmployeeTypeBrowse_ui_uiEmpty(prevState, prevAction, l_state) RETURNING errNo, actionNo
...
FUNCTION EmployeeTypeBrowse_ui_uiDisplay(p_prevState, p_prevAction, p_state)
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG EmployeeTypeBrowse_uidialog.EmployeeTypeBrowse_uidialog_record1_uiDisplay

        ON ACTION query
            {<POINT Name="fct.uiDisplay.dlg.onAction.query">} {</POINT>}
            LET m_actionNo = C_ACTION_QUERY

            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionQuery IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionQuery(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF

            CASE EmployeeTypeBrowse_events.m_DlgCtrlInstruction
        ...
        ON ACTION accept
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionAccept IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionAccept(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION cancel
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionCancel IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionCancel(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION close
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionClose IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionClose(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
        ...
        ON ACTION exit
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionExit IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionExit(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
        ...
        BEFORE DIALOG
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.CONTINUE_DIALOG
            IF EmployeeTypeBrowse_events.m_DlgEvent_Before_BrowseDialog IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_Before_BrowseDialog(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction, m_NextField)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction, m_NextField
            END IF
        ...
        AFTER DIALOG
            INITIALIZE EmployeeTypeBrowse_events.m_DlgCtrlInstruction TO NULL
            IF EmployeeTypeBrowse_events.m_DlgEvent_After_BrowseDialog IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_After_BrowseDialog(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION EmployeeTypeBrowse_ui_uiConstruct(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG EmployeeTypeBrowse_uidialog.EmployeeTypeBrowse_uidialog_record1_uiConstruct
        ...
        BEFORE DIALOG
            ...
            INITIALIZE EmployeeTypeBrowse_events.m_DlgCtrlInstruction TO NULL
            IF EmployeeTypeBrowse_events.m_DlgEvent_Before_SearchDialog IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_Before_SearchDialog(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
            CASE EmployeeTypeBrowse_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
        ...
        AFTER DIALOG
            ...
            INITIALIZE EmployeeTypeBrowse_events.m_DlgCtrlInstruction TO NULL
            IF EmployeeTypeBrowse_events.m_DlgEvent_After_SearchDialog IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_After_SearchDialog(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction, m_where, m_detailList)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction, m_where, m_detailList
            END IF
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION EmployeeTypeBrowse_ui_uiEmpty(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    DEFINE errNo INTEGER
    ...
    MENU
        BEFORE MENU
            ...
            END IF
        INITIALIZE EmployeeTypeBrowse_events.m_DlgCtrlInstruction TO NULL
        IF EmployeeTypeBrowse_events.m_DlgEvent_Before_EmptyDialog IS NOT NULL THEN
            CALL EmployeeTypeBrowse_events.m_DlgEvent_Before_EmptyDialog(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
        END IF
        ...
        ON ACTION query
            ...
            LET EmployeeTypeBrowse_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_MENU
            IF EmployeeTypeBrowse_events.m_DlgEvent_OnActionQuery IS NOT NULL THEN
                CALL EmployeeTypeBrowse_events.m_DlgEvent_OnActionQuery(DIALOG, m_state, EmployeeTypeBrowse_events.m_DlgCtrlInstruction)
                    RETURNING EmployeeTypeBrowse_events.m_DlgCtrlInstruction
            END IF
...
PUBLIC FUNCTION EmployeeTypeBrowse_ui_uiInitializeDefaultActions(p_actionList)
...
PUBLIC FUNCTION EmployeeTypeBrowse_ui_initializeDefaultUISettings()
