...
#+
PUBLIC FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_clear()
...
PUBLIC FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL OfferStatusConfigBrowse_uidialogdata_record1_clear()
    CALL OfferStatusConfigBrowse_uidata.OfferStatusConfigBrowse_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL OfferStatusConfigBrowse_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL OfferStatusConfigBrowse_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL OfferStatusConfigBrowse_uidata.OfferStatusConfigBrowse_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].offerstatuses_offerstatusid) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION OfferStatusConfigBrowse_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.offerstatuses_offerstatusid == m_record1_arrRecList[m_record1_keyIndex].offerstatuses_offerstatusid THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.offerstatuses_offerstatusid == m_record1_arrRecList[i].offerstatuses_offerstatusid THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
