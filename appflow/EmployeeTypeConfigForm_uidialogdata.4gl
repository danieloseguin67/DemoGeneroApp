...
#+
PUBLIC FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_clear()
...
PUBLIC FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL EmployeeTypeConfigForm_uidialogdata_record1_clear()
    CALL EmployeeTypeConfigForm_uidata.EmployeeTypeConfigForm_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL EmployeeTypeConfigForm_uidialogdata_record1_fetchRow() RETURNING errNo
...
#+
FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL EmployeeTypeConfigForm_uidata.EmployeeTypeConfigForm_uidata_record1_getDataByKey(m_record1_arrKeyRec[m_record1_keyIndex].employeetypes_emptypeid) RETURNING errNo, m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_setDefaultValues(p_dialog)
    ...
    IF EmployeeTypeConfigForm_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL EmployeeTypeConfigForm_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecGrid[1].*)
...
PUBLIC FUNCTION EmployeeTypeConfigForm_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF p_record1_br_uk.employeetypes_emptypeid IS NULL THEN
        LET idxReturn = 0
    END IF

    --If the seeking values match the current row, then no need to seek and return the current index
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.employeetypes_emptypeid == m_record1_arrKeyRec[m_record1_keyIndex].employeetypes_emptypeid THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.employeetypes_emptypeid == m_record1_arrKeyRec[i].employeetypes_emptypeid THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
