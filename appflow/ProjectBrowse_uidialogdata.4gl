...
#+
PUBLIC FUNCTION ProjectBrowse_uidialogdata_record1_clear()
...
PUBLIC FUNCTION ProjectBrowse_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL ProjectBrowse_uidialogdata_record1_clear()
    CALL ProjectBrowse_uidata.ProjectBrowse_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION ProjectBrowse_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL ProjectBrowse_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL ProjectBrowse_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION ProjectBrowse_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL ProjectBrowse_uidata.ProjectBrowse_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].projects_projectId) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION ProjectBrowse_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION ProjectBrowse_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.projects_projectId == m_record1_arrRecList[m_record1_keyIndex].projects_projectId THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.projects_projectId == m_record1_arrRecList[i].projects_projectId THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
