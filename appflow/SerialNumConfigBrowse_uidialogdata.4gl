...
#+
PUBLIC FUNCTION SerialNumConfigBrowse_uidialogdata_record1_clear()
...
PUBLIC FUNCTION SerialNumConfigBrowse_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL SerialNumConfigBrowse_uidialogdata_record1_clear()
    CALL SerialNumConfigBrowse_uidata.SerialNumConfigBrowse_uidata_record1_getDataArray(m_record1_arrRecList, m_record1_arrRecDB, p_where, p_detailList) RETURNING errNo
...
#+
FUNCTION SerialNumConfigBrowse_uidialogdata_record1_fetchRowAndDetails()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRowAndDetails.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRowAndDetails.init">} {</POINT>}
    CALL SerialNumConfigBrowse_uidialogdata_record1_fetchRow() RETURNING errNo
    CALL SerialNumConfigBrowse_uidialogdata_record1_fetchDetails()
    RETURN errNo
END FUNCTION
...
#+
FUNCTION SerialNumConfigBrowse_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL SerialNumConfigBrowse_uidata.SerialNumConfigBrowse_uidata_record1_getDataByKey(m_record1_arrRecList[m_record1_keyIndex].seqreg_sr_name) RETURNING errNo, m_record1_arrRecList[m_record1_keyIndex].*, m_record1_arrRecDB[m_record1_keyIndex].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION SerialNumConfigBrowse_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
PUBLIC FUNCTION SerialNumConfigBrowse_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.seqreg_sr_name == m_record1_arrRecList[m_record1_keyIndex].seqreg_sr_name THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.seqreg_sr_name == m_record1_arrRecList[i].seqreg_sr_name THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR
