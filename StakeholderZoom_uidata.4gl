...
PUBLIC FUNCTION StakeholderZoom_uidata_record1_getDataArray(p_arr_br_fields, p_arr_db_fields, p_where, p_detailList)
    --Declaration of parameters
    DEFINE p_arr_br_fields DYNAMIC ARRAY OF record1_br_type
    DEFINE p_arr_db_fields DYNAMIC ARRAY OF RECORD LIKE clientstakeholders.*
    ...
    LET l_sqlQuery = 'SELECT ', l_distinct
        ...
        , '   clientstakeholders.stakeholdername ASC '
    {<POINT Name="fct.record1_getDataArray.init">} {</POINT>}
    IF StakeholderZoom_events.m_DataEvent_record1_OnSelectRows IS NOT NULL THEN
        CALL StakeholderZoom_events.m_DataEvent_record1_OnSelectRows(l_sqlQuery)
    ...
    WHENEVER ERROR CONTINUE
        DECLARE getDataArray_cursqlsid_StakeholderZoom_record1 CURSOR FROM l_sqlQuery
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        RETURN SQLCA.SQLCODE
    END IF
    WHENEVER ERROR CONTINUE
        FOREACH getDataArray_cursqlsid_StakeholderZoom_record1 INTO
                p_arr_br_fields[i].clientstakeholders_stakeholderId,
                p_arr_br_fields[i].clientstakeholders_stakeholdername,
                p_arr_br_fields[i].clientstakeholders_clientId,
                p_arr_db_fields[i].*

            IF StakeholderZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
                CALL StakeholderZoom_events.m_DataEvent_record1_OnComputedFields(p_arr_br_fields[i])
            END IF
        ...
        FREE getDataArray_cursqlsid_StakeholderZoom_record1
    WHENEVER ERROR STOP
    IF SQLCA.SQLCODE != 0 THEN
        CALL p_arr_br_fields.clear()
        CALL p_arr_db_fields.clear()
...
PUBLIC FUNCTION StakeholderZoom_uidata_record1_getDataByKey(p_rec_br_uk)
    --Declaration of parameters
    ...
    WHENEVER ERROR CONTINUE
        SELECT @clientstakeholders.stakeholderId,
            @clientstakeholders.stakeholdername,
            @clientstakeholders.clientId,
            clientstakeholders.*
        INTO l_rec_BRdata.clientstakeholders_stakeholderId,
            l_rec_BRdata.clientstakeholders_stakeholdername,
            l_rec_BRdata.clientstakeholders_clientId,
            l_rec_DBdata.*
        ...
        IF StakeholderZoom_events.m_DataEvent_record1_OnComputedFields IS NOT NULL THEN
            CALL StakeholderZoom_events.m_DataEvent_record1_OnComputedFields(l_rec_BRdata)
        END IF
